...
public:
   ...
   *
   * This function requests the user to input the expense details: title,
   * amount, date, and an optional category. It then adds the constructed
   * expense to the provided list.
   *
   * @param[out] expenses Vector of expenses where the new expense will be
   * added.
   */
   ...
   *
   * Lists the expenses in a readable format including title, amount, date, and
   * category. If no expenses are present, a message indicating this will be
   * shown.
   *
  ...
  /**
   * @brief Allows users to edit an existing expense by providing a new title,
   * amount, date, and category.
   *
   * This function prompts the user to enter the index of the expense they want
   * to edit. It then allows the user to input new values for the expense's
   * title, amount, date, and category.
   *
   * @param[in,out] expenses Vector of expenses where the expense will be
   * edited.
   * @param[in] filterCategory of the expense to edit.
   */
   ...
   *
   * This function prompts the user to enter the index of the expense they want
   * to delete. It then removes the expense from the provided list.
   *
   * @param[in,out] expenses Vector of expenses where the expense will be
   * deleted.
   ...
   *
   * This function iterates through the provided list of expenses and sums up
   ...
   *
   * This function iterates through the provided list of expenses and searches
   * for expenses whose title or category contains the given query.
   *
   * @param[in] expenses Vector of expenses to search through.
   * @param[in] query The search query to match against expense titles and
   * categories.
   */
  void searchExpenses(const std::vector<Expense> &expenses,
   ...
   */
  bool saveExpenses(const std::vector<Expense> &expenses,
                    const std::string &filename) const;

  /**
   * @brief Loads the expenses from a file.
   *
   * This function reads the expenses from a file and adds them to the provided
   * list.
   *
   * @param[in,out] expenses Vector of expenses where the loaded expenses will
   * be added.
...
private:
  Expense createExpense(std::string &&title, double amount,
                          std::string &&category,std::string &&date);
