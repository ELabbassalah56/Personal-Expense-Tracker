...
void command_tracker::addExpense(std::vector<command_tracker_T> &expenses) {
  ...
  if (validateExpense(expense)) {
    expenses.push_back(createExpense(std::move(expense.title), expense.amount,
                                     std::move(expense.category), std::move(expense.date)));
...
void command_tracker::editExpense(std::vector<command_tracker_T> &expenses, int index) 
{
    ...
    std::cout << "Editing expense: " << oldExpenses.title << " | Amount: " << oldExpenses.amount
    ...
    if (validateExpense(newExpenses)) {
        oldExpenses = createExpense(std::move(newExpenses.title), newExpenses.amount,
                                   std::move(newExpenses.category), std::move(newExpenses.date));
        std::cout << "Expense updated successfully!" << std::endl;
    } else {
        std::cout << "Failed to update expense due to validation errors." << std::endl;
    }
